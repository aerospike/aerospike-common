...
void linear_histogram_dump( linear_histogram *h )
{
	...
	cf_debug(AS_INFO, "linear histogram dump: %s (%zu total)",h->name, h->n_counts);
	int i, j;
	int k = 0;
	...
	for (; i<=j;i++) {
		if (h->count[i] > 0) { // print only non zero columns
			...
			if (bytes <= 0) 
			{
				cf_debug(AS_INFO, "linear histogram printing error. Bailing ...");
				return;
			}
			pos += bytes;
		    if (k % 4 == 3){
		    	 cf_debug(AS_INFO, "%s", (char *) printbuf);
		    	 pos = 0;
		    	 printbuf[0] = '\0';
		    }
		    k++;
