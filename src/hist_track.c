...
get_start_row_n(cf_hist_track* this, uint32_t back_sec)
{
	...
	// Make sure when default query is run (e.g. latency:), we return atleast
	// valid last data instead of returing an error. This case happens when
	// the query is timed such that its right when histogram is being dumped
