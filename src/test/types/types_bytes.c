...
TEST( types_bytes_get_set, "as_bytes getting and setting" ) {
    ...
    assert_false( as_bytes_get(&b, test_len+2, &v, 1) ); // should fail out of bounds
    assert_false( as_bytes_get(&b, 0, &v, test_len+2) ); // should fail out of bounds
    assert_false( as_bytes_get(&b, test_len/2, &v, test_len) ); // should fail out of bounds

    // test a basic get
    assert( 1 == as_bytes_get(&b, 3, &v, 1) );
    assert( v == test_str[3]);
    ...
    v = 0;
    assert_true( as_bytes_get(&b, test_len-1, &v, 1) );
    assert( v == 0xff );
...
TEST( types_bytes_stack_append, "as_bytes append on stack" ) {
	...
	char str[27] = {0};
	uint32_t len = as_bytes_get(&b, 0, (uint8_t *) str, sizeof(str));
