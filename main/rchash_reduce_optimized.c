 ...
 *
 * README
 * THIS VERSION CREATED 6/4/2010 WHICH GREATLY REDUCES THE OVERHEAD OF REDUCES
 * FOR MANYLOCK HASH TABLES. IT SEEMS TO WORK GREAT BUT IS CONSIDERED TOO RISKY
 * GIVEN THAT WE'RE MID-DEPLOYMENT. SWITCH OVER TO THIS USE WHEN THE TIME IS RIGHT
 *
...
// in the case where we frequently reduce, it's important to narrow down the number
// of locks even for big tables - and only valid for "manylocks"
// 
// THIS IS NOT THREAD SAFE - do it right around when you create the table
//
...
rchash_set_nlocks(rchash *h, int n_locks)
{
	...
	for (uint i=0;i<h->lock_table_len;i++) {
		pthread_mutex_destroy( &(h->lock_table[i]));
	}
	cf_free(h->lock_table);
	h->lock_table = 0;

	h->lock_table = cf_malloc(sizeof(pthread_mutex_t) * n_locks);
	if (!h->lock_table) return(-1);
	for (uint i=0;i<n_locks;i++) {
		pthread_mutex_init( &(h->lock_table[i]), 0);
	}
	h->lock_table_len = n_locks;
	h->buckets_per_lock = h->table_len / n_locks;
...
rchash_put(rchash *h, void *key, uint32_t key_len, void *object)
{
	...
	if (h->flags & RCHASH_CR_MT_BIGLOCK) {
		...
		int lock_n = hash / h->buckets_per_lock;
		l = & (h->lock_table[lock_n]);
	}
...
//
// This is special enough to break out
//

void
rchash_reduce_manylock(rchash *h, rchash_reduce_fn reduce_fn, void *udata)
{
	// start with the zero lock held
	uint lock_id = 0;
	pthread_mutex_lock( &(h->lock_table[0] ));
	
	for (uint i=0; i<h->table_len ; i++) {

		uint new_lock_id = i / h->buckets_per_lock; // should be able to optimize this away
		if (new_lock_id != lock_id) {
			pthread_mutex_unlock( &(h->lock_table[lock_id] ));
			pthread_mutex_lock( &(h->lock_table[lock_id + 1]));
			lock_id++;
		}
...
rchash_reduce(rchash *h, rchash_reduce_fn reduce_fn, void *udata)
{
    ...
    if (h->flags & RCHASH_CR_MT_MANYLOCK) {
    	rchash_reduce_manylock(h, reduce_fn, udata);
    	return;
    }
...
// deleted
void
rchash_reduce_delete_manylock(rchash *h, rchash_reduce_fn reduce_fn, void *udata)
{
	// start with the zero lock held
	uint lock_id = 0;
	pthread_mutex_lock( &(h->lock_table[0] ));
...
rchash_reduce_delete(rchash *h, rchash_reduce_fn reduce_fn, void *udata)
{
	...
	if (h->flags & RCHASH_CR_MT_MANYLOCK) {
        rchash_reduce_delete_manylock(h,reduce_fn, udata);
        return;
    }
...
rchash_reduce_v(rchash *h, rchash_reduce_fn reduce_fn, void *udata)
{
	...
	if (h->flags & RCHASH_CR_MT_MANYLOCK) {
		rchash_reduce_manylock_v(h, reduce_fn, udata);
		return;
	}
...
// deleted
void
rchash_reduce_delete_manylock_v(rchash *h, rchash_reduce_fn reduce_fn, void *udata)
{
	// start with the zero lock held
	uint lock_id = 0;
	pthread_mutex_lock( &(h->lock_table[0] ));
	...
	for (uint i=0; i<h->table_len ; i++) {
		...
		while (list_he) {
			...
			if (cf_rc_count(list_he->object) < 1) {
				cf_info(CF_RCHASH,"rchash %p: internal bad reference count on %p",h, list_he->object);
				pthread_mutex_lock( &(h->lock_table[lock_id]));
				return;
			}
#endif		
...
rchash_reduce_delete_v(rchash *h, rchash_reduce_fn reduce_fn, void *udata)
{
	...
	if (h->flags & RCHASH_CR_MT_MANYLOCK) {
		rchash_reduce_delete_manylock_v(h, reduce_fn, udata);
		return;
	}
